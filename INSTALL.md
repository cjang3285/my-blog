...
### 3. 외부 네트워크에서 접속 (nginx 리버스 프록시)
```
https://183.101.163.146
```
- 🔐 첫 접속 시 로그인 필요 (`/admin/login`)
- 이후 7일간 세션 유지
- **권장**: 외부에서는 항상 nginx 리버스 프록시 사용 (HTTPS)
- 내부 IP:4321 직접 접속 대신 nginx를 통해 안전하게 접속

### 세션 쿠키 주의사항

접속 방법에 따라 별도의 세션이 생성됩니다:
- `http://172.30.1.95:4321`로 로그인 → 이 도메인에서만 유효
- `https://183.101.163.146`로 로그인 → 이 도메인에서만 유효

**권장 사용 패턴**:
- **집/사무실**: `http://172.30.1.95:4321` 사용
- **외부 네트워크**: `https://183.101.163.146` (nginx) 사용
